...
import com.cheating.inferno.displaymodes.InfernoWaveDisplayMode;
import com.cheating.inferno.displaymodes.InfernoZukShieldDisplayMode;
...
@Slf4j
public class InfernoPlugin extends Cheat
{
    private static final int INFERNO_REGION = 9043;

    @Inject
    protected InfernoPlugin(CheatingPlugin plugin, CheatingConfig configg)
    {
        super(plugin, configg);
    }
    ...
    {
        if (!"Cheating".equals(event.getGroup()))
        {
            return;
        }
        ...
        switch (infernoNPCType)
        {
            ...
            case ZUK:
                ...
                if (config.spawnTimerInfobox())
                {
                    ...
                    spawnTimerInfoBox = new InfernoSpawnTimerInfobox(itemManager.getImage(ItemID.TZREKZUK), plugin);
