package com.cheating.toa.Prayer;
...
public abstract class PrayerBoxOverlay extends Overlay {
    ...
    @Inject
    protected PrayerBoxOverlay(final Client client, final CheatingConfig config, final SpriteManager spriteManager)
    {
        this.client = client;
        this.config = config;
    ...
    private void updatePrayerComponent()
    {
        ...
        if (attack != null){
            prayerComponent.setBackgroundColor(!client.isPrayerActive(attack.getPrayer().getApiPrayer()) ? config.dangerColor() : STANDARD_BACKGROUND_COLOR);
