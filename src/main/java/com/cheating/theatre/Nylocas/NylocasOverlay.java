...
public class NylocasOverlay extends RoomOverlay
{
    ...
    @Inject
    protected NylocasOverlay(CheatingConfig config)
    {
        super(config);
    ...
    public Dimension render(Graphics2D graphics)
    {
        ...
        if (nylocas.isNyloActive())
        {
            ...
            for (NPC npc : npcMap.keySet())
            {
                ...
                {
                    if (config.nyloAggressiveOverlayStyle() == CheatingConfig.AGGRESSIVENYLORENDERSTYLE.TILE)
                    {
                    ...
                    else if (config.nyloAggressiveOverlayStyle() == CheatingConfig.AGGRESSIVENYLORENDERSTYLE.HULL)
                    {
                ...
                if (ticksLeft > -1 && ticksLeft <= config.nyloExplosionDisplayTicks())
                {
                    if (config.nyloTimeAlive() && !npc.isDead())
                    {
                        ...
                        if (config.nyloTimeAliveCountStyle() == CheatingConfig.NYLOTIMEALIVE.COUNTUP)
                        {
                        ...
                        {
                            if (config.nyloExplosionOverlayStyle() == CheatingConfig.EXPLOSIVENYLORENDERSTYLE.RECOLOR_TICK
                    ...
                    {
                        if (config.nyloExplosionOverlayStyle() == CheatingConfig.EXPLOSIVENYLORENDERSTYLE.TILE)
                        {
                            LocalPoint lp = npc.getLocalLocation();
                            if (lp != null)
                            {
