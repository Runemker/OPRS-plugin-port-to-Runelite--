package com.cheating.theatre.Xarpus;
...
public class Xarpus extends Room
{
    ...
    @Inject
    private CheatingPlugin plugin; //DO NOT USE. Just here for the exhumedCounter constructor.

    @Inject
    protected Xarpus(CheatingPlugin plugin, CheatingConfig config)
    {
        super(plugin, config);
    }
    ...
    public void init()
    {
        EXHUMED_COUNT_ICON = ImageUtil.loadImageResource(CheatingPlugin.class, "/Xarpus/1067-POISON.png");
    }
        ...
        if (xarpusActive)
        {
            ...
            if (o.getId() == GROUNDOBJECT_ID_EXHUMED)
            {
                ...
                if (exhumedCounter == null)
                {
                    ...
                    if (config.xarpusExhumedCount() != CheatingConfig.XARPUS_EXHUMED_COUNT.OFF)
                    {
                        exhumedCounter = new Counter(EXHUMED_COUNT_ICON, plugin, config.xarpusExhumedCount() == CheatingConfig.XARPUS_EXHUMED_COUNT.DOWN ? exhumedCount - 1 : 1);
                ...
                else
                {
                    ...
                    exhumedCounter.setCount(config.xarpusExhumedCount() == CheatingConfig.XARPUS_EXHUMED_COUNT.DOWN ? exhumedCounter.getCount() - 1 : exhumedCounter.getCount() + 1);
                }
