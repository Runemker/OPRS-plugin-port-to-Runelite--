...
import com.cheating.theatre.Direction;
...
public class XarpusOverlay extends RoomOverlay
{
    ...
    @Inject
    protected XarpusOverlay(CheatingConfig config)
    {
        super(config);
    ...
    public Dimension render(Graphics2D graphics)
    {
        ...
        if (xarpus.isXarpusActive())
        {
            ...
            if (config.xarpusLineOfSight() != CheatingConfig.XARPUS_LINE_OF_SIGHT.OFF)
            {
    ...
    private void renderLineOfSightPolygon(Graphics2D graphics)
    {
        ...
        if (xarpusNpc != null && (xarpusNpc.getId() == NpcID.XARPUS_8340 || xarpusNpc.getId() == 10768 || xarpusNpc.getId() == 10772) && !xarpusNpc.isDead() && xarpus.isPostScreech())
        {
            ...
            {
                Point[] points;
                boolean markMeleeTiles = config.xarpusLineOfSight() == CheatingConfig.XARPUS_LINE_OF_SIGHT.MELEE_TILES;
                switch (dir)
                {
                    case NORTHEAST:
