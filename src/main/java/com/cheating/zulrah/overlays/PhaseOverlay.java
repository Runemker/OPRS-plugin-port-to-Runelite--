...
import com.cheating.zulrah.constants.ZulrahType;
        ...
        extends OverlayPanel
{
    ...
    @Inject
    private PhaseOverlay(ZulrahPlugin plugin, CheatingConfig config)
    {
        this.plugin = plugin;
        this.config = config;
    ...
    {
        if (config.phaseDisplayTypeZulrah() == CheatingConfig.DisplayType.OFF || config.phaseDisplayTypeZulrah() == CheatingConfig.DisplayType.TILE || plugin.getZulrahNpc() == null || plugin.getZulrahNpc().isDead())
        {
            return null;
        }
        ...
        if (config.phaseRotationNameZulrah())
        {
    ...
    private InfoBoxComponent infoBoxComponent(Map.Entry<ZulrahType, MutablePair<String, Boolean>> entry)
    {
        ...
        infoBox.setOutline(config.textOutlineZulrah());
    ...
    private void displayRotationName(Graphics2D graphics)
    {
        ...
        textComponent.setOutline(config.textOutlineZulrah());
