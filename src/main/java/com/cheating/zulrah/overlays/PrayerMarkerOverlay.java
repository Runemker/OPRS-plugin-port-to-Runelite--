...
public class PrayerMarkerOverlay extends Overlay
{
    ...
    @Inject
    private PrayerMarkerOverlay(Client client, ZulrahPlugin plugin, CheatingConfig config)
    {
        this.client = client;
        this.plugin = plugin;
        this.config = config;
    ...
    {
        if (config.prayerMarkerZulrah() && plugin.getZulrahNpc() != null && !plugin.getZulrahNpc().isDead())
        {
            plugin.getZulrahData().forEach((data) -> {
                data.getCurrentPhasePrayer().ifPresent((prayer) -> {
                    if (client.getVarcIntValue(VarClientInt.INVENTORY_TAB) == 5)
                    {
                        ...
                        GUIOverlayUtil.renderWidgetPolygon(graphics, widget, color, true, false);
                    }
