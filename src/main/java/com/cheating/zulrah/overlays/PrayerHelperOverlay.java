...
public class PrayerHelperOverlay extends OverlayPanel
{
    ...
    @Inject
    private PrayerHelperOverlay(Client client, ZulrahPlugin plugin, CheatingConfig config, SpriteManager spriteManager)
    {
        this.client = client;
        this.plugin = plugin;
        this.config = config;
    ...
    {
        if (!config.prayerHelperZulrah() || plugin.getZulrahNpc() == null || plugin.getZulrahNpc().isDead())
        {
            return null;
        }
        Prayer prayer = null;
